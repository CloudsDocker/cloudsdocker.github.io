...
title: "iPhone 连 WireGuard 无法访问内网？6 小时踩坑后，真凶竟是 Windows NAT 配置！"
date: 2025-09-09
tags:
  - tech
  - WireGuard
...
可当我打开 Safari，输入公司内网可正常访问的 `oa.example.com` 并回车时，屏幕却始终停留在 —— 转圈加载，最终显示连接超时。
...
#### 数据包完整传输路径：
1. **客户端发起请求**：源 IP 为 10.8.0.100，目标地址为内网服务 `oa.example.com`
2. **隧道加密传输**：数据包通过 WireGuard 加密隧道发送至 Windows 服务端
3. **第一次 NAT 转换**：服务端将源 IP 从 10.8.0.100 修改为隧道 IP 10.8.0.1
4. **第二次 NAT 转换**：服务端再将源 IP 从 10.8.0.1 修改为自身内网 IP 192.168.1.100
5. **抵达目标服务**：内网服务器接收到的请求源 IP 为服务端内网 IP
...
我立即打开 Wireshark 监控 Windows 的以太网接口，同时用 iPhone ping 内网中可正常访问的 192.168.1.200 —— 结果显示无任何数据包通过物理网卡传输。
...
1. **流量识别**：标记源 IP 属于 10.8.0.0/24 的数据包为 "需 NAT 转换"
2. **接口选择**：自动匹配当前默认网关对应的物理接口（以太网 / WLAN）
3. **映射表建立**：记录隧道 IP: 端口与内网 IP: 端口的对应关系
4. **双向转发**：outbound 流量修改源 IP，inbound 流量修改目标 IP

当我执行完这条命令后，iPhone 立即成功访问到内网 OA 系统 —— 困扰 6 小时的问题终于解决。

## 第四个坑：AllowedIPs 配置的隐藏陷阱

解决 NAT 问题后，新的异常出现：部分内网服务可访问，部分无法打开。检查客户端配置后发现，`AllowedIPs` 参数设置存在疏漏：
...
多数人误解 `AllowedIPs` 为 "允许访问的 IP 白名单"，实际其核心作用是**配置客户端路由表条目**。

当配置为 `AllowedIPs = 10.8.0.0/24` 时，iPhone 会自动添加路由规则：
...
若需所有内网访问流量均通过 WireGuard 隧道，需将 `AllowedIPs` 配置为完整的内网网段：
...
当 iPhone 访问内网 Web 服务（如 `oa.example.com:80`）时，Windows NAT 模块会自动创建映射条目：
...
需重点关注三点：
1. **动态端口分配**：若指定端口被占用，NAT 会自动分配空闲端口
2. **状态跟踪**：实时记录连接状态，确保返回数据精准转发至客户端
3. **超时清理**：无活动连接会被自动删除，释放系统资源
...
该配置实现：访问服务端 `IP:33890` 时，自动转发至客户端 `10.8.0.100` 的 3389 端口（远程桌面）。
...
if (-NOT ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
    Write-Error "错误：必须以管理员身份运行此脚本！"
    exit 1
}

# 2. 检查IP转发
Write-Host "`n1. 检查IP转发状态..." -ForegroundColor Yellow
$ipForward = Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" -Name "IPEnableRouter"

if ($ipForward.IPEnableRouter -ne 1) {
    Write-Host "开启IP转发..." -ForegroundColor Red
    Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" -Name "IPEnableRouter" -Value 1
    Write-Host "✓ IP转发已开启（需重启电脑生效）" -ForegroundColor Green
} else {
